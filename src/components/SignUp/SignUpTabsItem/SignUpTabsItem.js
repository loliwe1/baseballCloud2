import React from 'react';
import PropTypes from 'prop-types';

const SignUpTabsItem = ({
  name,
  changeActiveTab,
  actvie,
  activeTabe,
}) => (
  <button
    type="button"
    className={`modal-signUp__btn modal-signUp__btn--${actvie} ${activeTabe}`}
    onClick={changeActiveTab}
  >
    <span className="modal-signUp__check-img">
      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="15" viewBox="0 0 14 15">
        <path
          fill="#FFF"
          fillRule="evenodd"
          d="M6.116 10.884l5.482-5.482a.566.566 0 0 0 0-.804l-.91-.91a.566.566 0 0 0-.804 0l-4.17 4.17L3.83 5.972a.566.566 0 0 0-.803 0l-.91.91a.566.566 0 0 0 0 .804l3.196 3.197c.223.223.58.223.803 0zM13.714 3v8.571a2.572 2.572 0 0 1-2.571 2.572H2.57A2.572 2.572 0 0 1 0 11.57V3A2.572 2.572 0 0 1 2.571.429h8.572A2.572 2.572 0 0 1 13.714 3z"
        />
      </svg>
    </span>
    <span className="modal-signUp__btn-label">{ name }</span>
  </button>
);

SignUpTabsItem.propTypes = {
  name: PropTypes.string.isRequired,
  changeActiveTab: PropTypes.func.isRequired,
  actvie: PropTypes.string.isRequired,
  activeTabe: PropTypes.string,
};

SignUpTabsItem.defaultProps = {
  activeTabe: '',
};

export default SignUpTabsItem;
